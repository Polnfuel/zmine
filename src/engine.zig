...
pub fn start_game(first_click: u16) !void {
    gen_field(first_click);
    playing = true;
    won = false;
...
fn gen_field(first_click: u16) void {
    game_field.fill(0);
    ...
    for (0..total_mines) |i| {
        var j: u16 = 0;
        while (true) {
            const a: u16 = @truncate(xorshift_rnd() % (field_size - i));
            j = @as(u16, @truncate(i)) + a;
        ...
        indices.set(j, tmp);
        game_field.set(indices.at(i), 12);
    }
    for (0..field_size) |cell| {
        if (game_field.at(cell) != 12) {
            game_field.set(cell, count_mines(@truncate(cell)));
        }
    }
